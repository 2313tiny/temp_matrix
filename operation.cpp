...
struct Matrix2{
	...
	void transpos( ){
		std::cout << "transpose matrix ... DONE\n ";
	}
};
...
{
	typedef true_type nshan;
};

template<typename I>
struct info< Matrix2<I> >
{
	typedef false_type nshan;	
};
...
template<typename T>
void fake_mult_matrix(T &from1, T & from2, T & to, true_type)
{
	for( int i = 0; i < from1.get_matrix_row(); ++i){	
	...
	std::cout << "A Version need to transpose matrix" << std::endl;
}

template<typename T>
void fake_mult_matrix(T &from1, T & from2, T & to, false_type)
{
	for( int i = 0; i < from1.get_matrix_row(); ++i){	
		for( int j = 0; j < from2.get_matrix_col(); ++j) {
		int sum = 0;
	...
	std::cout << "B Version you do not need anything" << std::endl;
}
